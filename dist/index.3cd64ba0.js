...
function drawModalSemester(index) {
    ...
    for(var i = 1; i < 3; i++){
        selected = "";
        if (i == semester.numSemester) selected = "selected";
    ...
    for(var i = new Date().getFullYear(); i < new Date().getFullYear() + 10; i++){
        selected = "";
        if (i == semester.year) selected = "selected";
    ...
    var field_year = '<select class="form-select" id="year" name="year" aria-label="Default select example"><option selected>Selecciona un a\xf1o</option>' + options_year + '</select><label for="floatingSelect">A\xf1o</label>';
    ...
    var field_description = '<textarea class="form-control" id="description" name="description" rows="4">' + semester.description + '</textarea><label for="floatingInput">Descripci\xf3n</label>';
    var field_opinion = '<textarea class="form-control" id="opinion" name="opinion" rows="4">' + semester.opinion + '</textarea><label for "floatingInput">Opini\xf3n</label>';
    ...
    form.addEventListener("submit", function(event) {
        ...
        if (typeof semestersData[newIndice] == "undefined") semestersData[newIndice] = new Object();
...
function eliminar(index) {
    // Eliminar semestre
    if (confirm("\xbfQuieres eliminar el semestre?")) $("#" + index).remove();
    else alert("No se ha eliminado el semestre");
}
function drawSelectSemester(semesters, selected = null) {
    ...
    if (selectSemestre) {
        ...
        if (searchParams.has("yearsemester")) $("#select-semestre").val(searchParams.get("yearsemester"));
        // seleccionamos la primera semestre por defecto si ninguna esta seleccionada
        if (!$("#select-semestre").val()) $("#select-semestre").val($("#select-semestre option:first").val());
    }
}
...
//# sourceMappingURL=index.3cd64ba0.js.map
